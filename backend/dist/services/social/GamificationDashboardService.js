...
exports.GamificationDashboardService = void 0;
const LeaderboardRepository_1 = require("../../repositories/LeaderboardRepository");
const db_1 = require("../../database/db");
class GamificationDashboardService {
    leaderboardRepo;
    constructor(leaderboardRepo = new LeaderboardRepository_1.LeaderboardRepository()) {
        this.leaderboardRepo = leaderboardRepo;
    }
    async getSummary(departmentId) {
        const now = new Date();
        // 1. Fetch CSR Activites and Volunteer Hours
        const volunteerAcks = await db_1.prisma.volunteerParticipation.findMany({
            where: {
                status: "APPROVED",
        ...
        const totalCSRActivities = await db_1.prisma.cSRActivity.count({
        ...
        const activeChallengesCount = await db_1.prisma.challenge.count({
            where: {
                status: "ACTIVE",
                startDate: { lte: now },
                endDate: { gte: now },
        ...
        const challengeParticipations = await db_1.prisma.challengeParticipation.findMany({
            where: {
                deletedAt: null,
                ...(departmentId ? { employee: { departmentId } } : {}),
            },
        ...
        const totalEmployeesCount = await db_1.prisma.employee.count({
        ...
        const badgeUnlocks = await db_1.prisma.employeeBadge.findMany({
            where: {
                deletedAt: null,
                ...(departmentId ? { employee: { departmentId } } : {}),
            },
        ...
        const recentRedemptions = await db_1.prisma.rewardRedemption.findMany({
            where: {
                deletedAt: null,
                ...(departmentId ? { employee: { departmentId } } : {}),
            },
        ...
        let leaderboard = await this.leaderboardRepo.findAll();
        if (departmentId) {
        ...
        const envGoals = await db_1.prisma.environmentalGoal.findMany({
            where: { deletedAt: null, ...(departmentId ? { departmentId } : {}) },
        });
        ...
        const issues = await db_1.prisma.complianceIssue.findMany({
            where: {
                deletedAt: null,
        ...
        const socialGoals = await db_1.prisma.socialGoal.findMany({
            where: { deletedAt: null, ...(departmentId ? { departmentId } : {}) },
        });
