import { CSRActivity, VolunteerParticipation, Prisma } from "@prisma/client";
...
import { CSRActivityRepository } from "../../repositories/CSRActivityRepository";
import { VolunteerParticipationRepository } from "../../repositories/VolunteerParticipationRepository";
...
export class CSRActivityService {
  private csrRepo: ICSRActivityRepository;
  private participationRepo: IVolunteerParticipationRepository;
  private goalRepo: ISocialGoalRepository;
  private gamificationService: GamificationService;

  constructor(
    csrRepo: ICSRActivityRepository = new CSRActivityRepository(),
    participationRepo: IVolunteerParticipationRepository = new VolunteerParticipationRepository(),
    goalRepo: ISocialGoalRepository = new SocialGoalRepository(),
  ...
  public async create(dto: CreateCSRActivityDto, callerUserId: string): Promise<CSRActivity> {
  ...
  public async findById(id: string): Promise<CSRActivity> {
    const activity = await this.csrRepo.findById(id);
    if (!activity) throw AppError.notFound(`CSR Activity with ID ${id} not found.`);
    return activity;
  }

  public async findAll(): Promise<CSRActivity[]> {
    return this.csrRepo.findAll();
  }

  public async update(id: string, dto: Partial<CreateCSRActivityDto>, callerUserId: string): Promise<CSRActivity> {
  ...
  public async delete(id: string, callerUserId: string): Promise<CSRActivity> {
    const oldState = await this.findById(id);
  ...
  public async join(activityId: string, proofUrl: string | null, callerUserId: string): Promise<VolunteerParticipation> {
    return prisma.$transaction(async (tx) => {
      const employee = await tx.employee.findFirst({
        where: { userId: callerUserId, deletedAt: null },
      });
      if (!employee) throw AppError.forbidden("Active employee profile required to join CSR activities.");

      const activity: any = await this.csrRepo.findById(activityId, tx);
      if (!activity) throw AppError.notFound(`CSR Activity with ID ${activityId} not found.`);

      // Check max participants limit
      if (activity.maxParticipants) {
        const activeCount = activity.participations.length;
        if (activeCount >= activity.maxParticipants) {
          throw AppError.conflict("This CSR Activity has reached its maximum participant limit.");
        }
      }

      const existing = await this.participationRepo.findByActivityAndEmployee(activityId, employee.id, tx);
      if (existing) throw AppError.conflict("You have already joined this CSR activity.");
  ...
  public async uploadProof(participationId: string, proofUrl: string, callerUserId: string): Promise<VolunteerParticipation> {
    return prisma.$transaction(async (tx) => {
      const employee = await tx.employee.findFirst({
        where: { userId: callerUserId, deletedAt: null },
      });
      ...
      if (!participation) throw AppError.notFound(`Volunteer participation record with ID ${participationId} not found.`);
      
      if (participation.employeeId !== employee.id) {
        throw AppError.forbidden("You can only upload proof for your own participations.");
      }
  ...
  public async approveParticipation(id: string, status: "APPROVED" | "REJECTED", proofStatus: "VERIFIED" | "REJECTED", callerUserId: string): Promise<VolunteerParticipation> {
    return prisma.$transaction(async (tx) => {
      const participation: any = await this.participationRepo.findById(id, tx);
      if (!participation) throw AppError.notFound(`Participation record with ID ${id} not found.`);
      if (participation.status === "APPROVED") throw AppError.badRequest("Participation is already approved.");
  ...
  public async findParticipationsByEmployee(employeeId: string): Promise<VolunteerParticipation[]> {
    return this.participationRepo.findByEmployeeId(employeeId);
  }

  public async findAllParticipations(): Promise<VolunteerParticipation[]> {
   ...
   */
  private async checkAndTriggerSocialGoalAchievements(departmentId: string, year: number, tx: Prisma.TransactionClient, callerUserId: string): Promise<void> {
